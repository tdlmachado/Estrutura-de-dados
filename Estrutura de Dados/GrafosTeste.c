...
void imprimeCaixaVertice(GRAFO *gr, int s){
    if (!gr) return;

    ADJACENCIA *ad = gr->adj[s].cab;
    char lines[64][64];
    int nlines = 0;
    ...
    printf("+");
    for(int i = 0; i < maxlen + 2; i++) printf("-");
    printf("+\n");

    // imprime conteúdo com padding
    for(int i = 0; i < nlines; i++){
        printf("| %s", lines[i]);
        int pad = maxlen - (int)strlen(lines[i]);
